...
const EditProductAdmin = ({ close, data: propsData, fetchProductData }) => {
  ...
  return (
    <section className="fixed top-0 left-0 right-0 bottom-0 bg-black/50 z-50 flex items-center justify-center">
      <div className="bg-white w-full p-4 max-w-2xl rounded mx-auto overflow-y-auto max-h-[95vh] ">
        <section className="">
          ...
          <div className="grid p-3">
            <form className="grid gap-2" onSubmit={handleSubmit}>
              ...
              <div>
                ...
                <div>
                  <label
                    ...
                    className="bg-neutral-100 border h-24 rounded flex items-center justify-center cursor-pointer"
                  >
                  ...
                  <div className=" flex flex-wrap gap-4">
                    {data.image.map((img, index) => {
                      return (
                        <div
                          key={img + index}
                          className=" h-20 w-20 mt-2 min-w-20 bg-blue-50 border relative group"
                        >
                          <img
                            src={img}
                            alt={img}
                          ...
                          <div
                            ...
                            className=" absolute bottom-0 right-0 p-1 bg-red-500 hover:bg-red-400 rounded cursor-pointer text-white hidden group-hover:block"
                          >
                            <MdDelete />
                          </div>
                        </div>
                ...
                <div>
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      const category = allCategorie.find(
                        (el) => el._id === value
                      );
                      ...
                      return (
                        ...
                        >
                          ...
                          <div
                            ...
                            onClick={() => handleRemovCategory(index)}
                          >
                            <IoClose size={20} />
                          </div>
                        </div>
                  ...
                  <div className="flex flex-wrap gap-3">
                    {data.subCategory.map((c, index) => {
                      return (
                        <div
                          key={c._id + index + "productsection"}
                          className="text-sm flex items-center gap-2 border p-1 bg-blue-50 mt-2"
                        >
                          <p className="">{c.name}</p>
                          <div
                            onClick={() => handleRemovSubCategory(index)}
                            className="bg-red-700 hover:bg-red-600 rounded-full transition-shadow text-white text-sm cursor-pointer"
                          >
                            <IoClose size={20} />
                          </div>
                        </div>
              ...
              <div
                ...
                className="  bg-white hover:bg-primary-200 by-2 px-3 w-32 font-semibold cursor-pointer border rounded border-primary-200 hover:text-white text-center"
              >
                Add Fields
              </div>

              <button className=" block border bg-white hover:bg-primary-200 hover:text-white rounded p-1 mt-2 font-semibold">
                {/* {imageloading ? <Loading /> : "Update Product"} */}
                Update Product
              </button>
            </form>
          </div>
