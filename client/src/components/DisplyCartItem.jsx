                  ...
                  <div className='min-h-[80vh] lg:min-h-[78vh] h-full  max-h-[calc(100vh-150px)] bg-blue-50 p-2 flex flex-col gap-4 '>
                    {/* display items */}
                    {
                      ...
                      {/* <p>{formatPrice(notDiscountTotalPrice -  totalPrice)}</p> */}
                        <p>{formatPrice(Math.abs(notDiscountTotalPrice - totalPrice))}</p>
                    </div>
                    <div className='bg-white rounded-lg p-4 grid gap-6 overflow-auto'>
                          {
                            cartItem[0] && (
                              cartItem.map((item , index)=>{
                                return (
                                  <div key={item?._id+"cartItemDesply"+index} className='flex w-full gap-4'>
                                      ...
                                      <div className='w-full max-w-sm '>
                                        ...
                                        <p className='text-xs'>
                                          {formatPrice(priceWithDiscount(item?.productId?.price ,item?.productId?.discount ))}
                                        </p>
                                      </div>
                                      <div>
                                        <AddToCartButton data={item?.productId} key={index} className=""/>
                                      </div>
                                  </div>
                                )
                              })
                                          ...
                                          <div className="flex items-center justify-between gap-4">
                                            <p>Items total</p>
                                            <p className=' line-through'>{formatPrice(-notDiscountTotalPrice)}</p>
                                          </div>
                        ...
                        <div className='bg-white flex flex-col justify-center items-center'>
                            ...
                            src={imageEmpty}
                             ...
                             className=' w-full h-full object-scale-down p-2'
                              />
                              <Link onClick={close} to={"/"} className='border text-blue-400 rounded shadow p-2'>
                                Shop Now
                              </Link>
                        </div>
                      )
